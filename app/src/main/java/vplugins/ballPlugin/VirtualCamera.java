...
public class VirtualCamera implements IXposedHookLoadPackage {
	...
	public void handleLoadPackage(final XC_LoadPackage.LoadPackageParam lpparam) throws Exception {
			...
			XposedHelpers.findAndHookMethod(cameraclass, "setPreviewTexture", SurfaceTexture.class, new XC_MethodHook() {
				...
				protected void beforeHookedMethod(MethodHookParam param) {
					...
					if (reallycamera != null && reallycamera.equals(param.thisObject)) {
						param.args[0] = VirtualCamera.virtual_st;
					...
					VirtualCamera.msurftext = (SurfaceTexture) param.args[0];


					if (VirtualCamera.virtual_st == null) {
						VirtualCamera.virtual_st = new SurfaceTexture(10);
					} else {
						VirtualCamera.virtual_st.release();
						VirtualCamera.virtual_st = new SurfaceTexture(10);
					}
					param.args[0] = VirtualCamera.virtual_st;

					if (VirtualCamera.msurf == null) {
						VirtualCamera.msurf = new Surface(VirtualCamera.msurftext);
					} else {
						VirtualCamera.msurf.release();
						VirtualCamera.msurf = new Surface(VirtualCamera.msurftext);
					}

					if (VirtualCamera.mMedia == null) {
						VirtualCamera.mMedia = new MediaPlayer();
					} else {
						VirtualCamera.mMedia.release();
						VirtualCamera.mMedia = new MediaPlayer();
					}

					VirtualCamera.mMedia.setSurface(VirtualCamera.msurf);
					...
					if (!(sfile.exists() && (!is_someone_playing))){
						VirtualCamera.mMedia.setVolume(0, 0);
						is_someone_playing =false;
					}else {
						is_someone_playing = true;
					}
					VirtualCamera.mMedia.setLooping(true);

					VirtualCamera.mMedia.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
						@Override
						public void onPrepared(MediaPlayer mp) {
							VirtualCamera.mMedia.start();
						}
					});

					try {
						VirtualCamera.mMedia.setDataSource(Environment.getExternalStorageDirectory().getPath() + "/DCIM/Camera1/virtual.mp4");
						VirtualCamera.mMedia.prepare();
					} catch (IOException e) {
						e.printStackTrace();
					}
		...
		XposedHelpers.findAndHookMethod("android.hardware.camera2.CaptureRequest.Builder", lpparam.classLoader, "addTarget", Surface.class, new XC_MethodHook() {
			...
			protected void beforeHookedMethod(MethodHookParam param) {
				...
				if ((!param.thisObject.equals(VirtualCamera.c2_builder))){
					VirtualCamera.c2_builder = (CaptureRequest.Builder) param.thisObject;
					...
					if (VirtualCamera.c2_image_reader == null) {
						VirtualCamera.c2_image_reader = ImageReader.newInstance(1280, 720, ImageFormat.YUV_420_888, 5);
					}
					...
					XposedBridge.log("CaptureRequest 添加目标："+param.args[0].toString());
					...
					XposedBridge.log("CaptureRequest 多个添加目标："+param.args[0].toString());
					param.args[0] = c2_image_reader.getSurface();
					XposedBridge.log(c2_virtual_surface.toString());
					process_camera2_play();
		...
		XposedHelpers.findAndHookMethod("android.hardware.Camera", lpparam.classLoader, "setPreviewDisplay", SurfaceHolder.class, new XC_MethodHook() {
			...
			protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
				if (!param.thisObject.equals(mcamera1)) {
					...
					VirtualCamera.mplayer1.setSurface(VirtualCamera.ori_holder.getSurface());
					is_hooked =true;
					mcamera1.setPreviewTexture(c1_fake_texture);
					File sfile = new File(Environment.getExternalStorageDirectory().getPath() + "/DCIM/Camera1/no-silent.jpg");
					if (!(sfile.exists() && (!is_someone_playing))){
						VirtualCamera.mplayer1.setVolume(0, 0);
						is_someone_playing =false;
					}else {
						is_someone_playing = true;
					}
					VirtualCamera.mplayer1.setLooping(true);

					VirtualCamera.mplayer1.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
						@Override
						public void onPrepared(MediaPlayer mp) {
							VirtualCamera.mplayer1.start();
						}
					});

					try {
						VirtualCamera.mplayer1.setDataSource(Environment.getExternalStorageDirectory().getPath() + "/DCIM/Camera1/virtual.mp4");
						VirtualCamera.mplayer1.prepare();
					} catch (IOException e) {
						e.printStackTrace();
					}
	...
	public void process_camera2_play(){
		if (c2_preview_Surfcae != null && (!is_c2_text_playing)){
			if (VirtualCamera.c2_player == null) {
				VirtualCamera.c2_player = new MediaPlayer();
			} else {
				VirtualCamera.c2_player.release();
				VirtualCamera.c2_player = new MediaPlayer();
			}
			is_c2_text_playing = true;
			VirtualCamera.c2_player.setSurface(VirtualCamera.c2_preview_Surfcae);
			File sfile = new File(Environment.getExternalStorageDirectory().getPath() + "/DCIM/Camera1/no-silent.jpg");
			if (!sfile.exists()){
				VirtualCamera.c2_player.setVolume(0, 0);
			}
			VirtualCamera.c2_player.setLooping(true);


			VirtualCamera.c2_player.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
				public void onPrepared(MediaPlayer mp) {
					VirtualCamera.c2_player.start();
				}
			});
			try {
				VirtualCamera.c2_player.setDataSource(Environment.getExternalStorageDirectory().getPath() + "/DCIM/Camera1/virtual.mp4");
				VirtualCamera.c2_player.prepare();
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		...
		if (c2_reader_Surfcae !=null && (!is_c2_surf1_playing)){
			...
			try {
				...
				c2_hw_decode_obj.set_surfcae(VirtualCamera.c2_reader_Surfcae);
		...
		if (c2_reader_Surfcae_1 !=null && (!is_c2_surf2_playing)){
			...
			try {
				...
				c2_hw_decode_obj_1.set_surfcae(VirtualCamera.c2_reader_Surfcae_1);
			...
			protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
				XposedHelpers.findAndHookMethod(param.args[0].getClass(), "createCaptureSession", List.class, CameraCaptureSession.StateCallback.class, Handler.class, new XC_MethodHook() {
					...
					protected void beforeHookedMethod(MethodHookParam paramd) throws Throwable {
						...
						XposedBridge.log("createCaptureSession 来HOOK了"+
								VirtualCamera.c2_image_reader.getSurface().toString());
						paramd.args[0] = Arrays.asList(VirtualCamera.c2_image_reader.getSurface());
					}
				});
			}
		...
		XposedHelpers.findAndHookMethod(nmb, "onPreviewFrame", byte[].class, android.hardware.Camera.class, new XC_MethodHook() {
			...
			protected void beforeHookedMethod(MethodHookParam paramd) throws Throwable {
				...
				if (localcam.equals(data_camera)) {
					...
					System.arraycopy(
							VirtualCamera.data_buffer, 0, paramd.args[0], 0, Math.min(VirtualCamera.data_buffer.length, ((byte[]) paramd.args[0]).length));
					//HookMain.data_buffer = null;
				} else {
					camera_callback_calss = nmb;
					repeat_count = 1000;
					VirtualCamera.data_camera = (android.hardware.Camera) paramd.args[1];
				...
				if (doRender) {
					...
					if (play_surf == null) {
						...
						if (outputImageFormat != null) {
							VirtualCamera.data_buffer = getDataFromImage(image, COLOR_FormatNV21);
						}
						image.close();
					}
