...
public class GLShaderProgram {
	...
	public void setProgram(String vertexSource, String fragmentSource)
	{
		...
		if(program != 0){
			...
			if(linkStatus[0] != GLES20.GL_TRUE){
				...
				destory();
				Log.e(Tag, "Link shader error:" + error);
				return;
			}
		}
	...
	public int getHandle(String name){
		...
		if(handle == -1){
			Log.e(Tag,"GLSL shader, Could not get attrib location for " + name);
		}else{
	...
	private int loadShader(int shaderType, String source){
		...
		if(shader != 0){
			...
			if(compiled[0] == 0){
				...
				Log.e(Tag,"Compile shader error:" + error);
				return -1;
			}
		}
	...
	private String loadRawString(int rawId, Context context){
        ...
        try{
			...
			Log.e(Tag,"loadRawString exception:" + ioe.getMessage());
        }
